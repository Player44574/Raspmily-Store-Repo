...
var YYFMOD_path = "user://The world editor " + str(version) +"/YYFMOD_x64.dll"
...
const clock_loader_10 = preload("res://Icons/clock_loader_10_24dp_E8EAED_FILL0_wght400_GRAD0_opsz24.png")
const clock_loader_20 = preload("res://Icons/clock_loader_20_24dp_E8EAED_FILL0_wght400_GRAD0_opsz24.png")
const clock_loader_40 = preload("res://Icons/clock_loader_40_24dp_E8EAED_FILL0_wght400_GRAD0_opsz24.png")
const clock_loader_60 = preload("res://Icons/clock_loader_60_24dp_E8EAED_FILL0_wght400_GRAD0_opsz24.png")
const clock_loader_80 = preload("res://Icons/clock_loader_80_24dp_E8EAED_FILL0_wght400_GRAD0_opsz24.png")
...
func _update_text():
	if version >= "1.3.3":
		if not file_exists(exe_path) or not file_exists(win_path) or not file_exists(options_path) or not file_exists(fmodL_path) or not file_exists(fmodStudioL_path) or not file_exists(YYFMOD_path):
...
func _verify_game_files():
	...
	if not directory.dir_exists("user://The world editor " + str(version) +"/"):
		directory.make_dir("user://The world editor " + str(version) +"/")
...
func _check_integrity():
	if !file_exists(exe_path):
		...
		$"../TextureRect".texture = clock_loader_10
		print("no exe")
		return
	...
	if !file_exists(options_path):
		...
		$"../TextureRect".texture = clock_loader_40
		print("no options")
		return
	if version >= "1.3.3":
		if !file_exists(fmodL_path):
			_download_file(fmodL_link, fmodL_path)
			$"../TextureRect".texture = clock_loader_60
			print("no fmodL")
			return
			
		if !file_exists(fmodStudioL_path):
			_download_file(fmodStudioL_link, fmodStudioL_path)
			$"../TextureRect".texture = clock_loader_80
			print("no fmodStudioL")
			return
			
		if !file_exists(YYFMOD_path):
			_download_file(YYFMOD_link, YYFMOD_path)
...
func _start_game():
	OS.shell_open(OS.get_user_data_dir() + "/The world editor " + str(version) +"/The world editor.exe")
...
func _on_OptionButton_item_selected(index):
	...
	YYFMOD_path = "user://The world editor " + str(version) +"/YYFMOD_x64.dll"
